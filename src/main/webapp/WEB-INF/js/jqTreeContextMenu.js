(function ($) {
	...
	$.fn.jqTreeContextMenu = function (menuElement, callbacks) {
		...
		// Handle the contextmenu event send from jqTree when user clicks right mouse button.
		$el.bind("tree.contextmenu", function (event) {
			...
			var selectNode = $el.tree("getSelectNode");
			if (selectNode !== event.node) {
		...
		this.disable = function () {
			if (arguments.length === 0) {
				...
				$menuEl.find("li > a").each(function () {
					var hrefValue = $(this).attr("href");
					var value = hrefValue.slice(hrefValue.ikndexOf("#") + 1, hrefValue.length);
					if ($.inArray(value, items) > -1) {
						$(this).closest("li").addClass("disabled");
						$(this).unbind("click");
					}
